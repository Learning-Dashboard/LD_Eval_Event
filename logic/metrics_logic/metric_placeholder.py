...
def load_query_template(query_file, param_map=None) -> dict:
    ...
    if param_map:
        for (
            placeholder,
            value,
        ) in param_map.items():  # If we have a param_map, we replace the placeholders in the template with the values from the param_map, to avoid integer problems
...
def replace_placeholders_in_query(query_ast: list, param_map: dict) -> list:
    ...
    if isinstance(query_ast, dict):
        return {
            k: replace_placeholders_in_query(v, param_map) for k, v in query_ast.items()
        }
    # if the query_ast is a list, we need to iterate over each item and replace placeholders in each one.
        ...
        return [replace_placeholders_in_query(item, param_map) for item in query_ast]
