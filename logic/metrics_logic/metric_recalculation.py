...
from logic.metrics_logic.run_mogo_query import (
    run_mongo_query_for_metric,
    evaluate_formula,
)
...
def compute_metric_for_student(
    metric_def: dict, event_type: str, student_name: str, team_name: str
) -> None:
    """
    Compute, evaluate and store an individual metric for a given student.
    """
    # Derive the .query filename from the .properties path
    basepath = os.path.splitext(metric_def["filePath"])[
        0
    ]  # strip ".properties" #HERE WE WILL PUT IT IN .ENV LATER
    ...
    logger.info(
        f"Recomputing INDIV metric '{metric_def['name']}' formula=({formula_str}) for student='{student_name}' team with external_id='{team_name}'"
    )
    ...
    doc = run_mongo_query_for_metric(
        team_name, student_name, query_file, event_type, param_map
    )
...
def compute_metric_for_team(
    metric_def: dict, event_type: str, team_name: str, students: list
) -> None:
    """
    ...
    if formula_str in ["stdevCommits", "stdevTasks"]:
        return compute_team_sd_metric(
            metric_def, event_type, team_name, collection_name, students
        )

    else:
        logger.info(
            f"Recomputing TEAM metric '{metric_def['name']}' formula=({formula_str}) for team with external_id='{team_name}'"
        )
        ...
        pipeline = load_query_template(
            query_file, param_map
        )  # Load the query template, we will replace the placeholders later if needed
...
def compute_team_sd_metric(
    metric_def, event_type, team_name, collection_name, team_members=None
):
    """
    ...
    query_file = basepath + ".query"
    logger.info(
        f"Recomputing metric '{metric_def['name']}' for team with external_id='{team_name}'"
    )
    ...
    if commitsTotal == 0:
        ...
        aggregator_doc = {"perUserCounts": aggregator_map, "teamMembers": team_members}
        store_metric_result(
            team_name=team_name,
            metric_def=metric_def,
            final_val=final_val,
