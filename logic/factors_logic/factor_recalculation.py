...
def latest_metric_value(team: str, metric_name: str, student: str = None) -> list:
    ...
    pipeline = [
        {"$match": {"metric": {"$regex": regex}, "student_name": {"$exists": True}}},
        {"$sort": {"student_name": 1, "evaluationDate": -1}},
        {"$group": {"_id": "$student_name", "latest": {"$first": "$value"}}},
    ]
...
def compute_factor(team_name: str, factor_def: dict, values_dict: dict) -> tuple:
    ...
    for m, tup_list in values_dict.items():
        for student, val in tup_list:
            ...
            flat_metric.append(
                m
            )  # List of the names of the metrics that compose the factors
            flat_student.append(
                student
            )  # List of the students names in case the factor uses an individual metric
    ...
    store_factor_result(
        ...
        intermediate_metric_values=values_dict,
    )
