...
def store_factor_result(
    team_name: str,
    factor_def: dict,
    final_value: float,
    intermediate_metric_values: dict,
) -> None:
    """
    ...
    weight_map = dict(zip(factor_def.get("metric", []), factor_def.get("weights", [])))
    ...
    for metric_root, tuples in intermediate_metric_values.items():
        ...
        for student, val in tuples:
            ...
            metric_parts.append(f"{label} (value: {round(val, 10)}, {weight_text})")
    ...
    static = {
        ...
        "createdAt": datetime.now(ZoneInfo("Europe/Madrid")).strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
    }
    ...
    dynamic = {
        ...
        "value": final_value,
        "info": info_field,
        "modifiedAt": datetime.now(ZoneInfo("Europe/Madrid")).strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
        "times_modified": n,
    }
    ...
    collection.update_one(
        {"_id": _id}, {"$set": dynamic, "$setOnInsert": static}, upsert=True
    )
