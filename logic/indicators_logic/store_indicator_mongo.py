...
def store_indicator_result(
    team_name: str,
    indicator_def: dict,
    final_value: float,
    intermediate_factor_values: dict,
) -> None:
    """
    ...
    weight_map = dict(
        zip(
            indicator_def.get(
                "factor", []
            ),  # en tu .properties suele llamarse "factor"
    ...
    for factor_name, tuples in intermediate_factor_values.items():
        ...
        for _student, val in tuples:
            ...
            factor_parts.append(f"{factor_name} (value: {round(val, 10)}, {wtxt})")
