...
logger.info(
    f"Event={event_type}, team with external_id={external_id}, students={students}"
)
...
for indicator_def in triggered_indicators:
    ...
    logger.info(
        f"Values of the factors of indicator {indicator_def['name']}: {indicator_values}"
    )
    final_val_indicator = compute_indicator(
